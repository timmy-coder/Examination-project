...
$selAns = $conn->query("SELECT * FROM exam_answer WHERE axmne_id='$exmne_id' AND exam_id='$exam_id' ");
...
{
	$updLastAns = $conn->query("UPDATE exam_answer SET exans_status='old' WHERE axmne_id='$exmne_id' AND exam_id='$exam_id'  ");
	if($updLastAns)
	{
		foreach ($_REQUEST['answer'] as $key => $value) {
			 $value = $value['correct'];
		  	 $insAns = $conn->query("INSERT INTO exam_answer(axmne_id,exam_id,quest_id,exans_answer) VALUES('$exmne_id','$exam_id','$key','$value')");
		}
		if($insAns)
		{
